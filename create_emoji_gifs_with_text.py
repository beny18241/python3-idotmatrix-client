#!/usr/bin/env python3
"""
Create animated emoji GIFs with text overlays for iDotMatrix calendar status
Creates FREE, CALL, and ERROR animated emojis with text
"""
...
def create_free_emoji_gif_with_text():
    """Create animated green checkmark/check emoji with 'FREE' text for FREE status"""
    ...
    for i in range(8):  # 8 frames for smooth animation
        ...
        # Add "FREE" text overlay
        if progress > 0.6:  # Text appears after checkmark
            text_progress = min(1.0, (progress - 0.6) / 0.4)
            try:
                ...
                # Draw "FREE" text with fade-in effect
                text = "FREE"
                ...
                text_color = (0, 255, 0) if alpha > 128 else (0, 128, 0)
                ...
                if text_progress > 0.5:
                    draw.text((8, 26), "FREE", fill=(0, 255, 0))
    ...
    frames[0].save(gif_path, save_all=True, append_images=frames[1:], 
                   duration=150, loop=0)
    print(f"✅ Created FREE emoji with text GIF: {gif_path}")
    return gif_path

def create_busy_emoji_gif_with_text():
    """Create animated red X or stop emoji with 'CALL' text for BUSY status"""
    ...
    for i in range(8):  # 8 frames for smooth animation
        ...
        # Add "CALL" text overlay
        if progress > 0.6:  # Text appears after X
            text_progress = min(1.0, (progress - 0.6) / 0.4)
            try:
                # Try to use a small font, fallback to default if not available
                font_size = 6
                ...
                # Draw "CALL" text with fade-in effect
                text = "CALL"
                text_bbox = draw.textbbox((0, 0), text, font=font)
                text_width = text_bbox[2] - text_bbox[0]
                text_height = text_bbox[3] - text_bbox[1]
                ...
                # Fade in effect
                alpha = int(255 * text_progress)
                text_color = (255, 0, 0) if alpha > 128 else (128, 0, 0)
                ...
                # Fallback: draw simple text without font
                if text_progress > 0.5:
                    draw.text((8, 26), "CALL", fill=(255, 0, 0))
    ...
    frames[0].save(gif_path, save_all=True, append_images=frames[1:], 
                   duration=150, loop=0)
    print(f"✅ Created BUSY emoji with text GIF: {gif_path}")
    return gif_path

def create_error_emoji_gif_with_text():
    """Create animated orange warning emoji with 'ERROR' text for ERROR status"""
    ...
    for i in range(8):  # 8 frames for smooth animation
        ...
        if progress > 0.6:  # Text appears after exclamation
            text_progress = min(1.0, (progress - 0.6) / 0.4)
            try:
                # Try to use a small font, fallback to default if not available
                font_size = 6
                ...
                # Draw "ERROR" text with fade-in effect
                text = "ERROR"
                text_bbox = draw.textbbox((0, 0), text, font=font)
                text_width = text_bbox[2] - text_bbox[0]
                text_height = text_bbox[3] - text_bbox[1]
                ...
                # Fade in effect
                alpha = int(255 * text_progress)
                text_color = (255, 165, 0) if alpha > 128 else (128, 82, 0)
                ...
                # Fallback: draw simple text without font
                if text_progress > 0.5:
                    draw.text((6, 26), "ERROR", fill=(255, 165, 0))
    ...
    frames[0].save(gif_path, save_all=True, append_images=frames[1:], 
                   duration=150, loop=0)
    print(f"✅ Created ERROR emoji with text GIF: {gif_path}")
    return gif_path

def main():
    """Create all emoji GIFs with text overlays"""
    print("🎬 Creating animated emoji GIFs with text overlays for iDotMatrix...")
    
    # Create all GIFs with text
    free_gif = create_free_emoji_gif_with_text()
    busy_gif = create_busy_emoji_gif_with_text()
    error_gif = create_error_emoji_gif_with_text()
    
    print("\n🎉 All emoji GIFs with text created successfully!")
    print(f"📁 FREE emoji with text: {free_gif}")
    print(f"📁 CALL emoji with text: {busy_gif}")
    print(f"📁 ERROR emoji with text: {error_gif}")
